    ...
    ColumnLayout {
      ...
      MonthGrid {
        ...
        delegate: Rectangle {
          ...
          color: (monthGrid.currDay == model.day && monthGrid.currMonth == model.month) ? Dat.Colors.color.secondary : Dat.Colors.color.surface_container_highest
          radius: 10

          Text {
            anchors.centerIn: parent
            color: {
              if (monthGrid.currDay == model.day && monthGrid.currMonth == model.month) {
                return Dat.Colors.color.on_secondary;
              } else if (parent.model.month == monthGrid.month) {
                return Dat.Colors.color.on_surface_variant;
              } else {
                // Add 70% opacity to the color for days from other months
                var baseColor = Qt.color(Dat.Colors.color.on_surface_variant);
                return Qt.rgba(baseColor.r, baseColor.g, baseColor.b, 0.70);
              }
            }
            font.family: Dat.Fonts.rye
            font.pointSize: 15
